...
// Modified and contributed by Nguyen Doan
...
int32_t Ymax,Ymin,X,Xmin,Xmax;// X goes from 0 to 127
...
int32_t minY, int32_t maxY, uint16_t bcolor){
  ...
  Ymin = minY;
  Ymax = maxY;
  ST7735_PlotClear(Ymin,Ymax);  // range from 0 to 4095
}
...
int32_t bufY[], uint16_t color){
  int32_t i, j, k;
  for(k = 0; k < num; k++){
    i = (127*(bufX[k]-Xmin))/(Xmax-Xmin);
    j = 32 + (127*(Ymax-bufY[k]))/(Ymax-Ymin);
    ST7735_DrawPixel(i,j,color);
    ST7735_DrawPixel(i+1,j,color);
    ST7735_DrawPixel(i,j,color);
    ST7735_DrawPixel(i+1,j+1,color);
  }
}
